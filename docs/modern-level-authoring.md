# Modern Level Authoring

This guide describes how to author true-color and animated terrain art for
OpenLiero TC packs.
...
The `MODERNLV` block adds an optional **display layer**: a parallel ARGB32
array of the same dimensions. When a pixel has an authored ARGB value
(`display_valid == 1`), the modern-mode renderer uses that colour directly
instead of looking up the palette. Classic-mode renderers ignore the display
layer entirely; they always use the palette index.
...
["MODERNLV" + display block + anim extension]  ← optional (display + anim)
```
...
| ramp_count     | 1                 | Number of animation ramps (0 = no anim layer) |
...
2. **Use an animation ramp** (`display_valid = 1`, `display_anim = N`): defines
...
### Worked example: animated water ramp
...
- Files without the `ramp_count` byte load fine — the loader treats
  end-of-stream after `display_valid` as `ramp_count = 0`, producing an
  empty anim layer.
- The MODERNLV block (including the anim extension) is forwarded over
  netplay (protocol v8) and embedded in replays (replay format v9). Peers or
  replay files without the anim extension load with an empty anim layer
  (static display only).
