...
void WObject::BlowUpObject(Game& game, int cause_idx) {
  ...
  fixed const kX = this->pos.x;
  fixed const kY = this->pos.y;
  fixed const kVelX = this->vel.x;
  fixed const kVelY = this->vel.y;

  game.wobjects.Free(this);

  if (w.create_on_exp >= 0) {
    common.sobject_types[w.create_on_exp].Create(game, Ftoi(kX), Ftoi(kY), cause_idx, fired_by,
                                                 this);
  }

  game.sound_player->Play(w.explo_sound);

  int const kSplinters = w.splinter_amount;

  if (kSplinters > 0) {
    if (w.splinter_scatter == 0) {
      for (int i = 0; i < kSplinters; ++i) {
        int const kAngle = game.rand(128);
        int const kColorSub = game.rand(2);
        common.nobject_types[w.splinter_type].Create2(game, kAngle, fixedvec(), fixedvec(kX, kY),
                                                      w.splinter_colour - kColorSub, cause_idx,
                                                      fired_by);
      }
    } else {
      for (int i = 0; i < kSplinters; ++i) {
        int const kColorSub = game.rand(2);
        common.nobject_types[w.splinter_type].Create1(
            game, fixedvec(kVelX, kVelY), fixedvec(kX, kY), w.splinter_colour - kColorSub,
            cause_idx, fired_by);
      }
    }
  }
...
void WObject::Process(Game& game) {
  ...
  if (common.h[HRemExp] && type - common.weapons.data() == LC(RemExpObject) - 1) {
  ...
  do {
    ...
    if (w.shot_type == 2) {
      fixedvec const kDir(cossin_table[cur_frame]);
      auto new_vel = kDir * w.speed / 100;

      if (owner->visible && owner->Pressed(Worm::kUp)) {
        new_vel += kDir * w.add_speed / 100;
      }

      vel = ((vel * 8) + new_vel) / 9;
    } else if (w.shot_type == 3) {
      fixedvec const kDir(cossin_table[cur_frame]);
      auto add_vel = kDir * w.add_speed / 100;
    ...
    if (w.part_trail_obj >= 0 && (game.cycles % w.part_trail_delay) == 0) {
      if (w.part_trail_type == 1) {
        ...
        common.nobject_types[w.part_trail_obj].Create2(
            game, kAngle, vel / LC(SplinterCracklerVelDiv), pos, 0, owner_idx, fired_by);
      }
    }
    ...
    for (std::size_t i = 0; i < game.worms.size(); ++i) {
      ...
      if ((w.hit_damage || w.blow_away || w.blood_on_hit || w.worm_collide) &&
        ...
        for (int i = 0; i < kBloodAmount; ++i) {
          int const kAngle = game.rand(128);
          common.nobject_types[6].Create2(game, kAngle, vel / 3, pos, 0, worm.index, fired_by);
        }

        if (w.hit_damage > 0 && worm.health > 0 && game.rand(3) == 0) {
          int const kSnd =
              game.rand(3) + 18;  // NOTE: MUST be outside the unpredictable branch below
          if (!game.sound_player->IsPlaying(&worm)) {
            game.sound_player->Play(kSnd, &worm);
          }
        }
