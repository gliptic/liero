...
struct BasicVec<T, 2>
{
	...
	constexpr BasicVec& operator+=(BasicVec const& r) { x += r.x; y += r.y; return *this; }
	constexpr BasicVec& operator-=(BasicVec const& r) { x -= r.x; y -= r.y; return *this; }
	constexpr BasicVec& operator*=(T s) { x *= s; y *= s; return *this; }
	constexpr BasicVec& operator/=(T s) { x /= s; y /= s; return *this; }
...
struct BasicRect
{
	...
	constexpr BasicRect(T x1_, T y1_, T x2_, T y2_)
	...
	friend BasicRect operator|(BasicRect a, BasicRect const& b) { a.join(b); return a; }
	friend BasicRect operator&(BasicRect a, BasicRect const& b) { a.intersect(b); return a; }
};
