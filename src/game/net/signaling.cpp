...
bool SignalingClient::Connect(const std::string& server_addr, uint16_t server_port) {
  ...
  enet_socket_set_option(kSock, ENET_SOCKOPT_IPV6_V6ONLY, 0);

  ENetAddress any_addr = {};
  any_addr.port = 0;
  memset(&any_addr.host, 0, sizeof(any_addr.host));
  enet_socket_bind(kSock, &any_addr);
...
void SignalingClient::ReportAddress(uint8_t addr_type, const std::string& ip, uint16_t port) {
  int const kIpLen = (addr_type == proto::kAddrIPv4) ? 4 : 16;
  std::vector<uint8_t> msg(1 + proto::kRoomCodeLen + 1 + 2 + kIpLen);
  msg[0] = proto::kReportAddr;
  std::memcpy(msg.data() + 1, roomCode_.data(), proto::kRoomCodeLen);
  msg[1 + proto::kRoomCodeLen] = addr_type;
  msg[1 + proto::kRoomCodeLen + 1] = static_cast<uint8_t>(port >> 8);
...
void SignalingClient::SendIceCredentials(const std::string& ufrag, const std::string& pwd) {
  ...
  msg[off++] = static_cast<uint8_t>(ufrag.size());
  std::memcpy(msg.data() + off, ufrag.data(), ufrag.size());
  off += ufrag.size();
  msg[off++] = static_cast<uint8_t>(pwd.size());
...
void SignalingClient::SendIceCandidate(const std::string& sdp_candidate) {
  ...
  std::memcpy(msg.data() + off, roomCode_.data(), proto::kRoomCodeLen);
  off += proto::kRoomCodeLen;
  auto const kCandLen = static_cast<uint16_t>(sdp_candidate.size());
  msg[off++] = static_cast<uint8_t>(kCandLen >> 8);
  msg[off++] = static_cast<uint8_t>(kCandLen & 0xFF);
...
void SignalingClient::Poll() {
  ...
  if ((state_ == kCreating || state_ == kJoining) && retryCount_ < kMaxRetries) {
    uint64_t const kElapsed = NowMs() - lastSendMs_;
    if (kElapsed >= static_cast<uint64_t>(kRetryIntervalMs)) {
      retryCount_++;
      uint8_t msg[1 + proto::kRoomCodeLen] = {};
...
void SignalingClient::HandleMessage(const uint8_t* data, size_t len) {
  ...
  switch (kType) {
    case proto::kRoomCreated: {
      ...
      if (off + 1 <= len) {
        uint8_t const kUserLen = data[off++];
        if (off + kUserLen + 1 <= len) {
          turnUser_ = std::string(reinterpret_cast<const char*>(data) + off, kUserLen);
          off += kUserLen;
          uint8_t const kPassLen = data[off++];
          if (off + kPassLen <= len) {
            turnPassword_ = std::string(reinterpret_cast<const char*>(data) + off, kPassLen);
          }
        }
      }
      state_ = kHosting;
