...
TEST_CASE("WeaponSelectSnap round-trip preserves state",
          "[rollback][weapsel]") {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    REQUIRE(restored.players[i].wormControlStates ==
            snap.players[i].wormControlStates);
  ...
  bool mutatedDiffers =
      mutated.players[0].menuSelection != snap.players[0].menuSelection ||
      mutated.localPrevInput != snap.localPrevInput;
  REQUIRE(mutatedDiffers);
}

TEST_CASE("Weapon select reaches StateGame in sync under zero jitter",
          "[rollback][weapsel]") {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < Settings::selectableWeapons; ++j) {
      REQUIRE(a->game.worms[i]->settings->weapons[j] ==
...
TEST_CASE("Weapon select transitions cleanly under jitter",
  ...
  INFO("a rollbacks=" << a->rollbackCount()
